...
{ namespace OS { namespace detail {
    ...
    {
        if (*scheduler != SCHED_OTHER && *scheduler != SCHED_FIFO && *scheduler != SCHED_RR && *scheduler != SCHED_SPORADIC) {
	...
	INTERNAL_QUAL int rtos_task_check_priority(int* scheduler, int* priority)
    {
        ...
        if (*scheduler == SCHED_OTHER) {
            ...
            if (*priority > 255){
                log(Warning) << "Forcing priority ("<<*priority<<") of thread with !SCHED_OTHER policy to 255." <<endlog();
                *priority = 255;
                ret = -1;
            }
        }
